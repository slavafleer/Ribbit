        ...
        public void onClick(DialogInterface dialog, int which) {
            switch (which) {
                ...
                case 2: // Choose picture
                    ...
                    choosePhotoIntent.setType("image/*"); // filter for images files
                    startActivityForResult(choosePhotoIntent, PICK_PHOTO_REQUEST);
                    break;
                case 3: // Choose video
                    Intent chooseVideoIntent = new Intent(Intent.ACTION_GET_CONTENT);
                    chooseVideoIntent.setType("video/*"); // filter for video files
        ...
        if(resultCode == RESULT_OK) {
            if(requestCode == PICK_PHOTO_REQUEST || requestCode == PICK_VIDEO_REQUEST) {
                ...
                if(requestCode == PICK_VIDEO_REQUEST) {
                    // Make shure that the file is less than 10MB
                    int fileSize = 0;
                    ...
                    try {
                        ...
                        Toast.makeText(this, getString(R.string.error_opening_file),
                                Toast.LENGTH_LONG).show();
                        return;
                    } finally { // Always must to close Streams at the end
                        try {
                            inputStream.close();
                    ...
                    if(fileSize >= FILE_SIZE_LIMIT) {
                        Toast.makeText(this, getString(R.string.error_file_size_to_large),
                                Toast.LENGTH_LONG).show();
                        return;
                    }
                }
